...
// import colors from 'colors'; // Didn't work

export class Responder extends Configurable(Component) {
  constructor(advertisement, discoveryOptions) {
    ...
    this.sock.on('message', (req, cb) => {
      ...
      if (this.listeners(req.type).length === 0 && this.discoveryOptions.logUnknownEvents) {
        this.discovery.log([this.advertisement.name, '>', `No listeners found for event: ${req.type}`]); // `No listeners found for event: ${req.type}`.yellow
      }
