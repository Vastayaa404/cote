...
class Discovery extends Discover {
    ...
    statusLogger(obj, status) {
        ...
        if (status) {
            const statusLog = status == 'online' ? '.online' : '.offline'; // const statusLog = status == 'online' ? '.online'.green : '.offline'.red;
            logs.push(this.advertisement.name, '>', obj.advertisement.type + statusLog); // logs.push(this.advertisement.name, '>', obj.advertisement.type.magenta + statusLog)
        } else {
            logs.push();
        }

        logs.push(
            `${obj.advertisement.name}${'#'}${obj.id}`); // `${obj.advertisement.name}${'#'.grey}${obj.id.grey}`

        if (obj.advertisement.port) {
            logs.push('on', obj.advertisement.port.toString());
        }
